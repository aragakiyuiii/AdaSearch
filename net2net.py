...
parser.add_argument('--period', type=str, default='b0', choices=['b0', 'b1', 'b2', 'b3'],
...
parser.add_argument('--n_cell_stages', type=str, default='3,3,5,5,6,3', help="maximum number possible of cells in each cell stage")
...
parser.add_argument('--b2', type=str, default='2,3,4,4,3,2', help="number of cells in b2 period")  # 18
...
parser.add_argument('--b1_width', type=str, default='32,48,104,136,224,256', help="number of cells in b1 period")
...
parser.add_argument('--net2normalnet', action='store_true', help="convert super_net to normal_net")
parser.add_argument('--net2deepernet', action='store_true', help="convert small super_net to deeper_net")
parser.add_argument('--net2widernet', action='store_true', help="convert small super_net to wider_net")
parser.add_argument('--next_deepernet_period', type=str, default='b1', choices=['b1', 'b2', 'b3'],
                    help='set net new deeper net. for example, you already have b0 searched super_net, now you set --next_deepernet_period b1 in order to get b1 net by net2deepernet')
parser.add_argument('--next_widernet_period', type=str, default='b1', choices=['b1', 'b2', 'b3'],
                    help='set net new wider net. for example, you already have b0 searched super_net, now you set --next_deepernet_period b1 in order to get b1 net by net2widernet')
parser.add_argument('--deepernet_period_n_cell_stages', type=str, default='2,2,3,3,4,1',
                    help="number of cells in this period")
parser.add_argument('--new_net_start_epoch', default=0, type=int,
                    help='if old net have run N epoch, new_net_start_epoch should be N')
parser.add_argument('--random_init', action='store_true', help='random init new deeper block or wider channel')
parser.add_argument('--save_block_alpha', action='store_true', help='save block alpha params in ckpt')
...
if args.unify_warmup_train:
    ...
    # search for large arch
        ...
        raise NotImplementedError
if args.net2deepernet:
    if args.next_deepernet_period == 'b0':
        assert args.next_deepernet_period != 'b0', 'b0 is smallest period. There is no smaller super_net to convert to b0 by net2deepernet'
    elif args.next_deepernet_period == 'b1':
        args.deepernet_period_n_cell_stages = args.b1
    elif args.next_deepernet_period == 'b2':
        args.deepernet_period_n_cell_stages = args.b2
    elif args.next_deepernet_period == 'b3':
        args.deepernet_period_n_cell_stages = args.b3
    else:
        raise NotImplementedError
...
if __name__ == '__main__':
    ...
    args.deepernet_period_n_cell_stages = [int(val) for val in args.deepernet_period_n_cell_stages.split(',')]
    # make sure your super_net have the same operation candidates, fix_depth or not
    ...
    try:
        ...
        console.write_log('Warning: Must load model! exception of load_model: %s' % e, prefix='fail_to_load_model')

    if args.net2deepernet:
        if args.random_init:
            arch_search_run_manager.net.net2deepernet(deeper_n_cell_stages=args.deepernet_period_n_cell_stages,
                                                      width_stages=args.width_stages,
                                                      stride_stages=args.stride_stages,
                                                      conv_candidates=args.conv_candidates,
                                                      fix_depth=args.fix_depth, n_cell_stages=args.n_cell_stages)
            arch_search_run_manager.net.init_model(run_config.model_init, run_config.init_div_groups)
            try:
                arch_search_run_manager.load_model()
            except Exception as e:
                console.write_log('Exception of load_model for random_init deeper block: %s' % e, prefix='fail_to_load_model')
            print('random init deeper net')
        else:
            arch_search_run_manager.net.net2deepernet(deeper_n_cell_stages=args.deepernet_period_n_cell_stages,
                                                      width_stages=args.width_stages,
                                                      stride_stages=args.stride_stages,
                                                      conv_candidates=args.conv_candidates,
                                                      fix_depth=args.fix_depth, n_cell_stages=args.n_cell_stages)
        print('already get deeper net')

    if args.net2widernet:
        wider_out_channel = get_out_channel_settings(args.next_widernet_period)
        wider_expand_rate = get_expand_rate_settings(args.next_widernet_period)
        print('wider_out_channel:', wider_out_channel)
        arch_search_run_manager.net.cpu()
        arch_search_run_manager.net.net2widernet(wider_out_channel=wider_out_channel, random_init=args.random_init, wider_expand_rate=wider_expand_rate)
        net2net_name = 'net2_wider_net_checkpoint.pth.tar'
        if args.save_block_alpha:
            ckpt = {k: v for k, v in arch_search_run_manager.net.state_dict().items() if 'alpha_out' not in k and 'alpha_expand' not in k}
        else:
            ckpt = {k: v for k, v in arch_search_run_manager.net.state_dict().items() if 'alpha_out' not in k and 'AP_path_alpha' not in k and 'alpha_expand' not in k}
        arch_search_run_manager.run_manager.save_model({
            'warmup': True,
            'epoch': args.new_net_start_epoch,
            'weight_optimizer': arch_search_run_manager.run_manager.optimizer.state_dict(),
            'arch_optimizer': arch_search_run_manager.arch_optimizer.state_dict(),
            'state_dict': ckpt
        }, model_name=args.next_widernet_period + net2net_name)
        print('already get wider net')


    if args.net2normalnet:
